/**
 * Logica de calculo - Logistica Virgilio
 */

const CONFIG = {
  // v21.17 - el 600 NO es un legajo de prueba descartable: es el legajo COMPARTIDO de
  // ENTREVISTAS y hace produccion REAL (persiste eventos y descuenta stock, v14.62).
  // Se saca de aca para que el candidato aparezca en el reporte; index.ts lo desdobla
  // por nombre (gv_nombre_prueba) antes de que estas cuentas agrupen por legajo.
  legajosTest: ['0', '999', '9999'],
  jornadaHs: 9,
  // v21.18 - la jornada es la VENTANA contra la que se recortan los pares. Por defecto es
  // 08:00-17:00, pero index.ts la pisa cuando el pedido trae desde_hora/hasta_hora, asi se
  // puede medir un lapso (13:00 a 16:30) sin tocar ninguna otra cuenta: todo lo que mide
  // horas pasa por splitParPorJornada. Los minutos van aparte porque 16:30 no es una hora.
  jornadaInicioHora: 8,
  jornadaInicioMin: 0,
  jornadaFinHora: 17,
  jornadaFinMin: 0,
  feriadosFijos: [
    '01-01', '03-24', '04-02', '05-01', '05-25', '06-17', '06-20',
    '07-09', '08-17', '10-12', '11-20', '12-08', '12-25'
  ],
  feriadosCustom: [],
  feriadosAPI: [],
  ...
  muertoNombres: { PB:'Bano', PC:'Almuerzo', Limp:'Limpieza', Perm:'Permiso' },
  ...
  filasPivot: [
    ...
    { key:'RT',    label:'Recep. Mercaderia', tipo:'op', rendType:'none' },
    { key:'RI',    label:'Recep. Insumos',  tipo:'op',   rendType:'none' },
    { key:'EI',    label:'Entrega Insumos', tipo:'op',   rendType:'none' },
    { key:'MG',    label:'Gondola',         tipo:'op',   rendType:'none' },
    { key:'CT',    label:'Conteo',          tipo:'op',   rendType:'none' },
    { key:'AT',    label:'Atendi Timbre',   tipo:'op',   rendType:'none' },
    ...
    { key:'PB',    label:'Bano',            tipo:'mu',   rendType:'none' },
...
function splitParPorJornada(start, end) {
  ...
  const JF = CONFIG.jornadaFinHora, JFm = CONFIG.jornadaFinMin || 0;
  const JI = CONFIG.jornadaInicioHora, JIm = CONFIG.jornadaInicioMin || 0;
  const cruzaDia = start.toDateString() !== end.toDateString();

  if (!cruzaDia) {
    const ini = new Date(start.getFullYear(), start.getMonth(), start.getDate(), JI, JIm, 0);
    const fin = new Date(start.getFullYear(), start.getMonth(), start.getDate(), JF, JFm, 0);
  ...
  const iniEnd = new Date(end.getFullYear(), end.getMonth(), end.getDate(), JI, JIm, 0);
  const finEnd = new Date(end.getFullYear(), end.getMonth(), end.getDate(), JF, JFm, 0);
...
function procesar(dataCruda, fechaDesde, fechaHasta) {
  ...
  prod = prod.map(r => ({
    ...
    dtInicio: parseDT(r.fechaIni, r.horaIni) || null,
  ...
  const segmentos = [];
  const cruces = [];
  const paresOriginales = [];

  const conteoActualPorLegajoDia = {};
  prod.forEach(r => {
  ...
  prod.forEach(r => {
    ...
    const k = `${r.legajo}__${r.fecha}`;
    fjEventos.push({
      legajo: r.legajo, fecha: r.fecha, hora: r.hora, dt: r.dt,
      reportado, actual: conteoActualPorLegajoDia[k] || {},
      eventsSnapshot, descripcion: r.descripcion
    });
  });
  ...
  function pushSegmentos(tipo, legajo, dtStart, dtEnd, extra) {
    ...
    if (cruzaDia || olvidado) {
      cruces.push({
        legajo, opcion: extra?.code || tipo, tanda: extra?.tanda || null,
        inicio: dtStart, fin: dtEnd, hsBruta, hsImputada, olvidado
      });
    }
    paresOriginales.push({
      tipo, legajo, code: extra?.code || null, tanda: extra?.tanda || null,
      inicio: dtStart, fin: dtEnd, hsBruta, hsImputada, olvidado, cruzaDia,
    ...
    splits.forEach(s => segmentos.push({
      tipo, legajo, fecha: s.fecha, dtIni: s.dtIni, dtFin: s.dtFin, hs: s.hs,
      hsDiaAnterior: s.hsDiaAnterior || 0, ...(extra || {})
    }));
  }
    ...
    function emparejarToggles(togs, onPar) {
      ...
      const marcarHuerfano = (a) => {
        cruces.push({
          ...
          inicio: a.dt, fin: a.dt, hsBruta: 0, hsImputada: 0, olvidado: true
        });
      };
      for (let i = 0; i < togs.length; i++) {
        ...
        const a = togs[i];
        if (j === -1) { marcarHuerfano(a); usados.add(i); continue; }
        const b = togs[j];
        ...
        if (cruzaDia) {
          ...
          for (let k = j+1; k < togs.length; k++) {
            ...
            if (togs[k].dt.toDateString() === diaB) { bTieneCompanero = true; break; }
          }
        }
        if (bTieneCompanero) { marcarHuerfano(a); usados.add(i); continue; }
        onPar(a, b);
  ...
  const reportes = Object.entries(agrup).map(([k, segs]) => {
    ...
    const JIm = CONFIG.jornadaInicioMin || 0, JFm = CONFIG.jornadaFinMin || 0;
    let llegadaTarde = 0, salidaTemprana = 0, gapHs = 0, tieneContinuacionDiaSig = false;
    if (primero) {
      const inicioJor = new Date(primero.getFullYear(), primero.getMonth(), primero.getDate(), JI, JIm, 0);
    ...
    if (ultimo) {
      const finJor = new Date(ultimo.getFullYear(), ultimo.getMonth(), ultimo.getDate(), JF, JFm, 0);
      ...
      tieneContinuacionDiaSig = segs.some(s => {
        ...
        const seg17 = new Date(s.dtFin.getFullYear(), s.dtFin.getMonth(), s.dtFin.getDate(), JF, JFm, 0);
    ...
    if (segs.length > 0 && primero && ultimo) {
      const iniJor = new Date(primero.getFullYear(), primero.getMonth(), primero.getDate(), JI, JIm, 0);
      const finJor = new Date(ultimo.getFullYear(),  ultimo.getMonth(),  ultimo.getDate(),  JF, JFm, 0);
  ...
  const crossDia = cruces
    ...
    .map(c => ({
      legajo: c.legajo, opcion: c.opcion, tanda: c.tanda || '',
...
function agruparPorPersona(reportes, getMt3Fn) {
  ...
  reportes.forEach(r => {
    ...
    g.pickHs += r.pickHs; g.armHs += r.armHs; g.ccHs += r.ccHs;
    g.opHs += r.opHs; g.muertoHs += r.muertoHs; g.totHs += r.totHs;
    g.dias.add(r.fecha);
...
function agruparPorSector(reportes, getMt3Fn) {
  ...
  function aportar(key, legajo, hs, pares) {
    ...
    s.hs += hs; s.operarios.add(legajo);
  ...
  return Object.entries(sectores).map(([nombre, s]) => {
    ...
    return { nombre, hs: s.hs, mt3, est, tandas: s.tandas.size, operarios: s.operarios.size };
  });
}
...
function agruparMuertos(reportes) {
  ...
  return Object.values(m).map(g => ({
    code: g.code, nombre: CONFIG.muertoNombres[g.code] || g.code,
    hs: g.hs, eventos: g.eventos, operarios: g.operarios.size
  })).sort((a,b) => b.hs - a.hs);
}
...
function agruparParaPivot(reportes, getMt3Fn, paresOriginales, fjEventos) {
  ...
  reportes.forEach(r => {
    ...
    g.pick.hs += r.pickHs; g.arm.hs += r.armHs; g.cc.hs += r.ccHs;
    ...
    g.opHs += r.opHs; g.muHs += r.muertoHs; g.totHs += r.totHs;
  ...
  (fjEventos || []).forEach(fj => {
    ...
    g.FJ.dias.push({
      fecha: fj.fecha, hora: fj.hora,
      reportado: fj.reportado || {}, actual: fj.actual || {},
      eventsSnapshot: fj.eventsSnapshot || [], mismatches
    });
    g.FJ.hayFJ = true;
  });
  return out;
