...
>
> Última actualización: 2026-06-30 · Versión app al documentar: **v5.15**
>
> Nota: **datos (sin bump) — `Racks_Capacidad`: máximos por celda (#5, PROVISIONAL)**. Tabla **`Racks_Capacidad(id, sector, linea, cod_art, maximo)`** = **capacidad (máximo de cajas) por celda de rack**, para la importación/reconciliación de racks (#5). `sector` = celda física (A01…F28), `linea='LK'` (constante), `cod_art` = el código **rotulado en la celda** (se guarda **tal cual el rótulo**, incl. ceros a la izquierda: 026/027/029/030), `maximo` = la columna impresa **"Cjas Total"** (la capacidad; el conteo manuscrito "Total de Cj por estantería" se **ignora**). PK **surrogada `id`** (no `sector`) porque hay celdas con **2 códigos** (A64/A67/A73/A78/C09/C14/C15/C19/C20). Cargadas **278 filas** (celdas A01–F28 de las **9 hojas** fotografiadas; algunas celdas con 2 códigos); **4 sin máximo** (rótulo en blanco): **A62, C10, C14, E20**. Celdas **Libres** omitidas (A80/A83/C01/D35/E15). ⚠ **PROVISIONAL** — el dueño va a pasar el listado final en **Excel** (faltan hojas) y ahí se reconcilia. El `maximo` servirá para **topear "a pedir"/reposición** y para **reconciliar el saldo de racks** (las bajas `baja_racks` dejaron racks negativos en 583E/584E/503E/598E "hasta arreglarlo") contra el conteo. Sin cambios de código ni de app.
>
