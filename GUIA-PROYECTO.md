...
>
> Última actualización: 2026-07-24 · Versión app al documentar: **v5.98**
>
> Nota: **v5.98 — Facturación (NPs a FC): tabla en % → nunca más scroll horizontal**. El fix de v5.91 (box-sizing + max-width 1400) no alcanzaba: con `table-layout:fixed` y anchos en **px**, si el monitor es más angosto que la suma de anchos, la tabla no achica y desborda (barra de scroll, columna Acción cortada). Ahora las 12 columnas usan **anchos en %** que suman 100% (5/5/5/4/15/13/17/3.5/10/11/4/7.5): con `width:100%` + fixed la tabla **siempre** entra en el contenedor y las columnas achican proporcionalmente; el texto largo (Razón Social, Dirección) se corta con "…". `overflow-x:auto` queda solo de red de seguridad y no se dispara. Entra completa en cualquier monitor sin scroll horizontal.
>
> Nota: **v5.97 — PPP: Parque Avellaneda → Zona 1 y ruta Sur/Centro/Oeste (Z2 se suma a Z1·Z3·Z4)**. Dos cambios de ruteo pedidos por el dueño. (1) **Barrio**: "Parque Avellaneda" pasa de *Zona 3 - CABA Oeste* a ***Zona 1 - CABA Sur***. Se hizo en la tabla autoritativa **`Zonas_Barrios`** (`barrio_norm='parque avellaneda'`, gana sobre el hardcodeado y toma efecto sin re-deploy) **y** en el fallback hardcodeado `PPP_BARRIO_ZONA` (index.html) para que queden consistentes. `pppZonaDeBarrio` = override localStorage → `Zonas_Barrios` (Supabase) → `PPP_BARRIO_ZONA`. (2) **Ruta**: **Zona 2 (CABA Centro) ahora va junto con Z1·Z3·Z4** en la ruta **"Sur / Centro / Oeste"** (antes iba sola como "Centro"). Se actualizaron los 4 lugares: `_pppRuta` (código `so` ahora incluye Z2, se elimina `c`), el resumen de camiones (`surOeste` incluye Z2, `nCam` deja de contar `centro`), `RT_RUTAS` (el optimizador de ruta fusiona Centro en Sur/Centro/Oeste) y las leyendas/colores (chip `rt-c` deja de usarse; los pedidos de Z2 se colorean como Sur/Centro/Oeste). Z4 (GBA Sur) sigue en ese grupo como estaba. Las columnas por zona del resumen (Z1…Z7) no cambian; solo la agrupación por camión. Validado JS + grep de residuos.
>
> Nota: **v5.96 — Fix del drenaje por NP (v5.95): usar `ref=tanda|NP` + remanente real**. Al aplicar v5.95 apareció la restricción **`mov_stock_pipeline_dedup` = unique(upper(ref), upper(cod_art), deposito, tipo)**: solo permite **un** `facturado` por (ref, cod, depósito). Como v5.95 drenaba con `ref=tanda`, **dos NP que comparten un código** (ej. C86C: 706 y 825 están en 44498 y 44497) chocaban → la 2ª NP no drenaba. Corrección: `stockSalidaFacturadoNP` ahora usa **`ref=tanda|NP`** (ref propio por NP, no chocan) y el tope se calcula con **`_stockAfacturarRestanteTanda`** (nuevo), que suma el `a_facturar` de la tanda cruzando `ref=tanda` (separado) + `ref=tanda|NP` (drenajes) — `_stockNetoDepTanda` (ref exacto) no los vería. El barrido final también usa el remanente real. Validado con los datos reales de C86C (15 códigos, incluidos los compartidos, drenan a 0) y un test de estrés de tope (2 NP piden 8 de un código con 13 → 8+5, sin negativo). **Backfill de C86C aplicado**: 44497 y 44498 ya estaban facturadas antes de v5.95, así que se insertaron a mano los `facturado` que las drenan (15 códigos → `a_facturar` de C86C en 0, sin negativos); el aviso "STOCK ESTANCADO" de C86C se apaga. La NP vacía 44500 no drena nada y ya no traba.
>
> Nota: **v5.95 — El stock de "a facturar" se drena POR NP (no espera al 100% de la tanda)**. Antes, `a_facturar` solo salía del stock cuando la tanda quedaba **100% facturada** (`stockSalidaFacturado`, dispara al tickear el último NP). Problema real (C86C): una NP **vacía** (0 líos, ej. 44500) o simplemente sin facturar **trababa el stock ya facturado de las otras NP** → aviso "STOCK ESTANCADO" eterno. Ahora `facTickNP`, en **cada** tick, llama **`stockSalidaFacturadoNP(np, tanda)`**: saca de `a_facturar` los artículos de **esa** NP según su **composición TAL** (`_talComposicionNP` parsea `NP|LÍOS|TANDA|A=codXn,…|LIO`), con **tope** al neto disponible de la tanda (nunca deja negativo) y **dedup por NP** (movimiento `tipo='facturado'`, `ref=tanda`, **`ubicacion=NP`**). Al llegar al 100% queda un **barrido final** (`stockSalidaFacturado`, ahora dedup con `ubicacion='__barrido__'`) como red de seguridad por si la composición no cubrió todo. Una NP con 0 líos drena `{}` y no traba nada. Validado el parser+tope con los datos reales de C86C (drena los 9 artículos de 44497+44498, la NP vacía 44500 no drena nada, sin negativos). ⚠ Los NP ya facturados **antes** de esta versión no se drenan retroactivamente (necesitan un barrido/reconciliación puntual, ej. C86C).
>
...
>
> Nota: **v5.92 — Guardarraíles al aprobar bajadas de racks (evita el racks negativo tipo 931E/984E)**. Contexto: el 21/07 los saldos de `racks` de 931E (−1) y 984E (−2) quedaron negativos porque un operario registró **dos bajadas del mismo código el mismo día** (12 cajas + 1, y 24 + 2), donde la segunda era la **misma mercadería contada como "1 MC"** metida en el campo *cajas*; `racksAprobar` no validaba saldo y las aprobó ambas → racks negativo (`terminado` inflado). El campo del operario ya pide **solo cajas (inner)**, así que el fix va en la **aprobación** (`racksAprobar`, `index.html`). Nuevo helper **`racksBajadaAlerta(b, saldos)`** que detecta 2 condiciones: **(1) TOPE** — la bajada dejaría `racks` negativo (`pedir > saldo`), y **(2) DUPLICADO** — ya hay otra bajada del mismo `cod_art` el mismo día AR (`_rkArDay`, sirve aunque `orden_id` sea null, como en el incidente). Si alguna aplica, `racksAprobar` muestra un **`confirm`** con el detalle (saldo, cuánto quedaría, la conversión 1 MC = N cajas) antes de mover stock; no es bloqueo duro (Marianela puede aprobar igual, ej. stock físico > contado). Además, en la lista *"Bajadas propuestas por operarios"* (`stkBodyRacks`) cada fila con alerta muestra un **badge rojo** ("⚠ dejaría racks en −N · ya hay otra bajada de este código hoy"). Solo lado admin; no cambia la pantalla del operario ni el modelo de datos. Corrección de los saldos ya negativos: 4 movimientos `tipo='ajuste'` ref `fix-dup-baja-racks` (racks +1/+2, terminado −1/−2) → 931E y 984E en 0.
>
