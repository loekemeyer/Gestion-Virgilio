...
>
> Nota: **v7.66 — Guardado a Góndola (MG) deja de ser un toggle: entrar/salir sin el botón
...
> `tests/mg-reentrada.cjs`. Bump **v7.66**.
>
...
> `SURVIVING_TOGGLES` (`["CR","RR"]`; MG salió en v7.66), `TOGGLE_CODES`, `NEVER_INPUT`, `INC_TOGGLE`/`INC_DESC`
...
- `TOGGLE_CODES = [CR, RR, CC, RT, RI, EI, AT, PB, Limp, PC, Perm, CT]` — abren y cierran. (MG salió en **v7.66**: pasó a módulo directo tipo CP/SC.)
...
- `SURVIVING_TOGGLES = [CR, RR]` — sobreviven la medianoche; el resto se autocierra. (MG salió en **v7.66**: ya no es toggle.)
