...
>
> Nota: **v6.09 — CP (Completar Pedido) lado operario: solo muestra faltantes cuyo artículo está "a guardar"** (parte A del rediseño pedido por el dueño). Antes `showCPModal` listaba **todos** los faltantes (`Entregas_Virgilio.cajas_falto>0`, NP no facturada). Ahora **filtra** a los que tienen stock en el depósito **`a_guardar`** (`e._saldo.a_guardar > 0`) — la idea es: si llegó mercadería para guardar y hay una NP que la necesita, completala; si el artículo no está a guardar, no figura. El filtro se aplica en `showCPModal` y `cpReloadFaltantes`; **excepción**: cuando se entra enfocado en una NP puntual (`focusNp`, desde "Cargar las cajas" del pop-up de faltante) **no** se filtra, para no romper ese acceso directo. Cada fila muestra ahora **"a guardar: N"**. El flujo del operario (tocar NP → cantidad → lío existente/nuevo, origen a_guardar) queda igual. Pendiente parte B: vista read-only para la **operadora** dentro de **Facturación** y **bloqueo del tilde** en Facturación (NP con faltante recuperable de a_guardar/góndola; override solo operadora + alerta Telegram).
>
> Nota: **v6.08 — Reconciliación sobre v6.07 (trae los cambios de la rama `stock-negativo`, renumerados)**. La rama `claude/stock-negativo-931e-984e-17hdht` se había bifurcado en v5.90 y usó v5.91→v5.98 en paralelo a `main` (que llegó a v6.07 con OTRAS features). Se hizo `merge origin/main → rama`: **cero conflictos de código** (tocan áreas distintas), solo chocaban las líneas de versión. Todo el trabajo de la rama queda sobre v6.07 bajo **v6.08**. Incluye: (a) **Facturación NPs a FC**: tabla con anchos en **%** (nunca scroll horizontal) + `box-sizing:border-box`. (b) **Racks — guardarraíles al aprobar bajada** (`racksBajadaAlerta`: tope de saldo + aviso de duplicado + saldo "en racks hoy" por fila) y **botón "✗ Rechazar"** (`racksRechazar`, estado `rechazada` habilitado en el check de `Racks_Bajadas`). (c) **Drenaje de `a_facturar` POR NP** (`stockSalidaFacturadoNP` con `ref=tanda|NP` + `_stockAfacturarRestanteTanda`, barrido final de red). (d) **PPP**: Parque Avellaneda → Zona 1 (Zonas_Barrios + hardcodeado) y **Z2 (Centro) en la ruta Sur/Centro/Oeste** con Z1·Z3·Z4. Correcciones de datos ya live (931E/984E, C86C, 982E #28, 97822). *Nota: los números de nota v5.91–v5.98 de abajo, si aparecieran, corresponden a las features de `main`, no a las de esta rama.*
>
