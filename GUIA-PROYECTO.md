...
>
> Nota: **stock (sin bump) — Racks sembrado desde la planimetría · unidad = MASTER CAJAS (#5)**. Regla del dueño: *"cuando bajan, bajan en master cajas siempre"* → el depósito **`racks`** de `Movimientos_Stock` se lleva en **master cajas** (NO inner). Se sembró 1 ajuste por código (`tipo='ajuste'`, `ref='seed_planimetria_racks_2026-06-30'`, **reversible**: borrando esos movs) para que `saldo racks = master de la planimetría` (`Racks_Planimetria`, filas `ocupado`, LK+CH). **53 códigos**, total **1642 master**; reconcilió los negativos de las `baja_racks` (583E −48→**65**, 584E→40, 598E→171, 503E→22). **4 huérfanos sin tocar** (saldo viejo, NO figuran en la planimetría → a revisar/zerar): 363E=6, 366E=4, 536E=15.67, 585E=60. El **inner** se deriva por `Cajas_x_Master` (hay que poblarlo desde la planimetría: inner/master por código). ⚠ **Pendiente de CÓDIGO**: el módulo operario **"Bajar de racks" (v5.15)** hoy baja en cajas sueltas → debe pasar a **master cajas** y, al bajar a góndola, **convertir** master→inner (`racks −X master` / `terminado +X·Cajas_x_Master`).
>
