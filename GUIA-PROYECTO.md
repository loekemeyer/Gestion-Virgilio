...
>
> Última actualización: 2026-07-22 · Versión app al documentar: **v5.65**
>
> Nota: **v5.65 — Botón "Descargar excedentes (Excel)" en Stock y Compras**. En la solapa **📊 Stocks** (arriba, bajo Stock/Ingresos/Salidas) un botón `stkExportExcedentes()` que baja un **`.xls`** (tabla HTML con MIME `application/vnd.ms-excel` + BOM UTF-8 → lo abre Excel; **sin librería**, cliente-side, no toca Supabase) con el saldo ACTUAL del depósito `excedente` por código: **Código · Descripción · Cajas · Ubicación** + fila TOTAL. Saldo vía `stockComputeSaldos(_stk.movs, cutoff, asOf)` (mismo corte/vista que la tabla); ubicaciones de las entradas (`delta>0`, campo `ubicacion`). Al documentar: **39 códigos / 2078 cajas** (501=746 en N11/N3 — el guardado a excedente ya persiste tras el fix v5.64). Nueva en smoke.
>
