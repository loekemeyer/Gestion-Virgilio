...
- **Propuesta original**: una tabla `Gondola_Celdas(emp, sector, cod, orden_picking, cajas_max)`. **DESCARTADA** tras investigación: `Planimetria` (1 cod → 1 sector+orden, picking order) y `Capacidad_Sector` (1 cod → N sectores, capacidad por estante) modelan conceptos distintos con cardinalidades incompatibles — unificarlas en una sola tabla es arquitecturalmente incorrecto.
- **Resolución (2026-08-28)**: bulk-load de las 340 entradas de `planimetria.js` a la tabla `Planimetria` (360 filas totales con overrides); `planimetria.js` pasa a cache offline; `Capacidad_Sector` se mantiene separada; `Capacidad_Gondola` ya fue eliminada. Fuentes reducidas de 4 a 2 (tabla completa + .js offline). **Hecho** — `sql/planimetria_carga_base.sql`.
...
- `PPP_Programacion_Diaria.zona` (columna del Sheet, por NP) **y** `Zonas_Barrios` (barrio→zona, diccionario, GUIA:7140; index.html:26102-26111). **Divergencia VISTA y ya monitoreada**: el evento `PPE` reporta `zonadif:N` (zona de la fila ≠ zona del diccionario) y `sinzona:N` (GUIA:7331). **Resolución (2026-08-28)**: ya formalizado — `Zonas_Barrios` es canónica (83 filas), trigger `trg_ppp_autozona` auto-completa zona vacía desde el diccionario Y auto-aprende barrios nuevos con zona geográfica, `trg_ppp_zona_canonica` normaliza la escritura, RPC `zona_barrio_set` persiste asignaciones manuales. PPE reconcilia activamente. Fallback offline `PPP_BARRIO_ZONA` sincronizado con Supabase (84→100 entradas, 5 zonas corregidas: burzaco, lanús, lugano, villa lugano, villa luro). **Hecho**.
...
|---|---|---|---|---|
| Programación diaria + base de pedidos | Google Sheet PPP (`1-16YXe0…`) | `PPP_Programacion_Diaria`, `PPP_Base_Pedidos` | ✅ **Pull server-side** (2026-08-28): funciones Postgres `sync_ppp_programacion_diaria()` (gviz CSV) y `sync_ppp_base_pedidos()` (`/export?format=csv` para preservar artículos tipo 035E) + pg_cron cada 5 min (`sql/sync_ppp_pull_server_side.sql`). Reemplaza el Apps Script push (`apps-script/sync-ppp-supabase.gs`, deprecado). | Bajo: pull directo sin credentials externas; parseo unificado con el patrón de `sync_ppp_entregados_meta` |
...
| Precios de venta LK | LK `products.list_price/uxb` | `precios_venta` | ✅ **AUTOMATIZADO** (2026-08-28): Edge Fn `sync-precios-venta` + pg_cron diario 06:00 ART (`sql/sync_precios_venta.sql`). También sincroniza `cob_uxb_lk`. Receta manual conservada como fallback en `plata_perdida.sql`. | Bajo: cron diario; divergencia máxima 24 h vs EN VIVO de `arca-wsfe/preciar` (aceptable) |
...
| uxb padrón LK completo | LK products ∪ loke_products | `cob_uxb_lk` (`sql/cobranzas.sql:82`) | ✅ **AUTOMATIZADO** (2026-08-28): incluido en Edge Fn `sync-precios-venta` | Bajo: se sincroniza junto con `precios_venta` |
...
1. **Semana 1 (todo bajo riesgo)**: ~~TOP-6 (docs/SQL stale)~~ ✅ (refs a `PPP_Pedidos_Entregados` marcadas obsoletas en `ppp_supabase.sql`, `MIGRACION-SUPABASE-PPP.md`, `PLAN-INTEGRACION-ISIS-5547.md`), ~~TOP-10 (key en 1 archivo + corregir GUIA)~~ ✅ (GUIA actualizada: 6 archivos con la anon key), ~~TOP-5 (`EQUIV_FAMILIAS` del front)~~ ✅ (v11.101/v11.102: loader desde tabla `Equivalencias_Familia` con semilla conservada como fallback offline), ~~4.3/4.4 (limpiar np/tanda en el sync)~~ ✅ (triggers `trg_norm_ppp_base`, `trg_norm_ppp_prog_np`, `trg_norm_ppp_meta_np` + `fn_norm_tanda` con upper; aplicado 2026-08-28), ~~legajos test a `Empleados.tipo`~~ ✅ (Fase A: función `es_legajo_test`; Fase B: 7 vistas SQL migradas; Fase B-front: 4 inline checks en index.html centralizados a `esLegajoPrueba()` + 3 en `generar_reporte_agentes_v2.sql` a `es_legajo_test()`; Fase C — UPDATE Empleados.tipo: pendiente, requiere backup + permiso explícito), ~~metas a `Stock_Config`~~ ✅ (ya migradas: `meta_m3h_picking`/`meta_m3h_armado` en `Stock_Config`, localStorage es solo cache offline).
2. **Semanas 2-3**: ~~TOP-8 (cron de precios)~~ ✅ (Edge Function `sync-precios-venta` + pg_cron diario 06:00 ART; sincroniza `precios_venta` y `cob_uxb_lk` desde LK automáticamente; Chef sigue manual — falta secret `CHEF_SERVICE_KEY` en el vault), ~~TOP-4 (`empresa_de_np` + `Codigos_Duales` en SQL)~~ ✅, ~~TOP-7 (unificar sync PPP en pull server-side)~~ ✅ (funciones Postgres `sync_ppp_programacion_diaria()` gviz CSV + `sync_ppp_base_pedidos()` `/export?format=csv` para preservar 035E; pg_cron `*/5 * * * *`; Apps Script push deprecado — `sql/sync_ppp_pull_server_side.sql`), ~~fichadas a trigger server-side~~ ✅ (Phase 1+2: `trg_fichadas_virgilio_espejo` + `trg_registros_fichada_espejo`; front ya no escribe en `Fichadas_Historico`), ~~tabla `Sync_Estado` con alerta~~ ✅ (`watchdog_syncs_externos` lee `cron.job_run_details`).
3. **Mes 2**: ~~TOP-2 (canon de `cod_art` en el write + backfill)~~ ✅ (función standalone `canon_cod_art_val(text)` + índice funcional en OC_Maximos + 5 trigger functions por nombre de columna + 10 triggers en tablas target + backfill 143 filas en 4 tablas; `Importados_Volumen` usa `cod` no `cod_art`; aplicado 2026-08-28 — `sql/canon_cod_art_extendido.sql`), ~~TOP-3 (`Planimetria`/`Gondola_Celdas` única)~~ ✅ (bulk-load de las 340 entradas de `planimetria.js` a la tabla `Planimetria` — 360 filas totales, ON CONFLICT preservó las 22 overrides existentes; `planimetria.js` pasa a cache offline; `Capacidad_Sector` se mantiene separada: modela capacidad 1:N, no picking order 1:1; bug fix trigger `Articulos_Discontinuados` con función correcta `fn_canon_col_cod()`; aplicado 2026-08-28 — `sql/planimetria_carga_base.sql`), ~~1.11 (zona)~~ ✅ (ya formalizado: `Zonas_Barrios` es canónica con auto-learn trigger `trg_ppp_autozona` + RPC `zona_barrio_set`; zona de PPP se deriva automáticamente; PPE reconcilia zonadif/sinzona; fallback offline `PPP_BARRIO_ZONA` sincronizado con Supabase 84→100 entradas, 5 correcciones — aplicado 2026-08-28).
