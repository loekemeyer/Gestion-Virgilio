...
| **Prender la numeración** | ✅ **Lista para prender.** Se revisó la lógica entera y se midió: credencial de LK cargada, `pwebNumerar()` inalcanzable, seeds en 1, tablas en 0. Apareció y se arregló un agujero (v12.88): el job y "A Programar" no filtraban `enviado_a_compras` y habrían programado 30 días de pedidos que ISIS ya entregó (208 de 213). Ahora **pendiente = no enviado a compras**; el job lee 10 NP de LK y 1 de Chef. ⚠ **El interruptor de verdad está en LK**: el cron `procesar-pedidos-web` (12:30) tiene que apagarse el mismo día. Checklist completo, en orden, en `SUPABASE-GESTION-VIRGILIO.md` §3.l | dueño |
