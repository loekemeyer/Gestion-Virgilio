...
| 6 | Basura en LK `bot_customer_whatsapps` | ✅ 07/09: eran las 5 filas de la tabla y **todas eran WhatsApp de prueba** (dueño: *"fueron wpp de prueba"*, *"209 y 217 también son de prueba"*, *"borralos"*). Borradas; la tabla quedó vacía. No se corrigió la basura de `empresa`/`cod_cliente`: se borró todo. Backup en `sql/backups/bot_customer_whatsapps_20260907_pre_borrado.sql`. |
| 7 | Pedidos de clientes LK cargados por Chef | ✅ Decidido 07/09: es un pedido de Chef, se factura por Chef, artículo LK con "L" (505L); v13.71 valor a lista. ✅ v13.72: vendedor del padrón Chef en el Excel, Excel partido en ISIS LK / ISIS CH, doble contra ISIS LK detectado por CUIT+fecha (`en_produccion_lk`, cartel en A Programar), RPC buena de Chef en el front. Informe `docs/INFORME-PEDIDOS-LK-POR-CHEF.md`. ✅ v13.75 (dueño 07/09: *"no es problema que sean clientes de Loeke y de Chef; los que le hacemos FC E vendiéndole art. de Loeke —buscá en sales_lines— son los que no van"*): motivo `cliente_fc_lk` — cliente con FC Electr. en ISIS LK (art. de Loeke) en los últimos 180 días (`doble_lk_dias`) → su pedido web de Chef no se programa, lo tipea compras. Fuentes: `sales_lines` (LK, `gv_cuits_con_fc_lk`) + `isis_lk.documentos` (Virgilio). **v13.76: por CUIT, no por código** (dueño: *"el cod cliente no significa nada"*). 154 de los 357 clientes con código en las dos empresas entran. **v13.77: esa regla se APAGÓ (`doble_lk_dias = 0`)**: "FC E" era **Factura E de Tierra del Fuego** (la emite Chef). Regla real (dueño): pedido de la página LK con entrega en Tierra del Fuego → se arma como Loeke con **L** en cada artículo y el Excel ISIS va a **Chef** con el código de cliente de Chef (por CUIT). Los 10 clientes LK de TdF ya cruzan por CUIT. Vive en la vista `v_pedidos_web` de LK (`isis_empresa`, `cod_isis`); §3.bb. |
...
| 11 | Checklist manual ISIS al facturar por Chef con artículos de Loeke | ✅ v13.78 (dueño 07/09): panel 🧾 en Facturación por NP con códigos L — ajuste − en ISIS LK, ajuste + en ISIS CH, con tilde y legajo (`GV_Fac_Ajustes_ISIS`, `gv_fac_ajustes_isis`). GV descuenta el stock de Loeke solo (drenaje con L arreglado). Lo que sigue siendo manual: hacer los dos ajustes en ISIS. |
...
| ~~**8743 · Pedido facturado visible en la página y sin edición**~~ | ✅ **Sábado 05/09, LK.** Vista `gv_pedido_web_estado_pagina` (Virgilio) → FDW → RPC `gv_estado_mis_pedidos` (LK) → "Mis pedidos" con stepper de 4 pasos; facturado/entregado bloquea Editar (front y `edit_order_fast`). Chef afuera (sin acceso). Detalle §3.t | hecho (LK) |
| ~~**Sacar el prefijo `GV-` de las tandas**~~ | ✅ **Sábado 05/09 (backend, sin bump).** `PPP_Web_Config.tanda_prefijo = ''`. `ppp_web_proxima_letra()` = 4 → la primera tanda de Gestión es **`E01A`** (Producción dejó D71A). 0 tandas web previas, nada que renombrar. Volver: `valor_texto = 'GV-'` | hecho |
| ~~**Control de remito = entregado, solo**~~ | ✅ **v12.95 (2026-09-05).** Dueño: *"cuando se controla el remito tiene que pasar directo a Pedidos Entregados, sin que nadie toque nada; en Gestión automático"*. Vista `gv_ppp_entregados` (toda NP con CRN, ISIS o web). Programación la esconde, Pedidos Entregados la muestra aunque no esté facturada. Sin Excel. Detalle §3.r | hecho |
| **Pedido muerto: Chef 200 (El Martillo, 13/08, 33 líneas)** | Salió por mail a ISIS el 13/08 y no está en Producción ni facturado. Control de los últimos 50 clientes por página (§3.q): todo lo demás está en Producción, en camino (⏳ 12 enviados jue/vie/sáb, entran el lunes) o en Gestión (LK 1350). Chef 208 lo cargó ISIS como LK (98544/98545). **Decide el dueño**: cargarlo en ISIS a mano o ponerlo en Gestión (habría que marcarlo `enviado_a_compras_at = null` en Chef, proyecto sin acceso). Repetir el control el lunes a la tarde | dueño |
...
que faltan son del dueño: cargar `KRIKOS_IMAP_PASS` en el Vault de LK y **mergear la rama a
`main`** (el merge es lo que despliega la Bandeja).
