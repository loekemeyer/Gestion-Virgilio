...
- [x] **5290** (2026-07-31, hecho 2026-08-01 v6.89) — **Impresora automática de etiquetas por lío**. El **programa emite solo** cada etiqueta **al cerrar cada lío** (v6.89; apenas confirman la pilita, para ensuncharla en el acto). Cada etiqueta = **composición (cod+cajas) + razón social abreviada** (tanda/NP/operario; lugar de entrega ⏳ pendiente de parametrizar). — _hecho (v6.89, **piloto** detrás del switch `vir_etiqueta_lio` + sólo legajo 0/1): al cerrar cada lío (`_compAddLio`) la app encola en `Etiquetas_Lio` (Supabase) su etiqueta con el ZPL; el imprimidor `tools/imprimir-etiquetas-lio.ps1` (PowerShell, RAW) corre en la PC de la **Zebra S4M** y la saca sola. ⚠ Si borran un lío ya impreso, la etiqueta ya salió. ⏳ Falta: lugar de entrega + probar con la impresora real; después ampliar a legajos reales y (a futuro) micro-PC + impresora por puesto._
