...
## v15.69 — El pedido web se parte: lo disponible por un lado, lo que espera mercadería por el otro (2026-09-11)

> El número saltó a **v15.69**: otra sesión tomó v15.67 y v15.68 el mismo día. En los
> comentarios del SQL aplicado y en `sql/gv_ppp_web_diferido.sql` puede quedar escrito "v15.67".
...
## §3.cw — Una sola fuente de UxB: la tabla `GV_UxB` y la vista `gv_uxb_resuelto` (v16.14) — 2026-09-12

Thomas, 12/09: ***"quiero que empieces a borrar las tablas de uxb en supabase para que quede solo 1,
pero claramente a medida que ya estén sin uso. Las que tengan uno, empezá a cambiarles la búsqueda
para que solo busquen en 1 sola."***

Esta versión pone los **dos cimientos**. No borra ni migra nada todavía: eso va cuando esté cargado
el listado que va a pasar (Loeke y Chef).

### `public."GV_UxB"` — la tabla canónica

`empresa ('LK'|'CH')` · `cod` · `uxb` · `descripcion` · `origen` · `actualizado`. **PK (empresa, cod)**,
RLS prendida, `select` para anon.

La clave es lo importante: hay **21 códigos que son DOS PRODUCTOS distintos** según la empresa, y
ninguna tabla vieja podía representarlos.
...
### `public.gv_uxb_resuelto` — el único lugar donde preguntar

Devuelve `(empresa, cod, uxb, fuente)`. Primero mira `GV_UxB`; si ese código todavía no está
cargado, cae en cascada a las viejas: **`Articulos_Cajas` (filtrada POR MARCA)** → `OC_Maximos`
activo → maestro → `precios_venta` → `cob_uxb_lk` → `proyeccion_madre`.
...
- **cada paso tiene desempate explícito**, así que el resultado **no cambia entre corridas**
  (§3.cv: el 026 devolvió 12 y minutos después 36 sin que nadie tocara nada).

Cobertura al crearla, con `GV_UxB` vacía — **973 filas**:
...
La columna `fuente` es el **termómetro de la migración**: cuando diga `GV_UxB` para todos, las
viejas quedan sin uso y recién ahí se borran.

### El plan, en orden

| | Paso | Estado |
|---|---|---|
| a | tabla `GV_UxB` + vista `gv_uxb_resuelto` con fallback | ✅ v16.14 |
| b | cargar el listado de Thomas (Loeke y Chef) | esperando |
| c | diff de `gv_uxb_resuelto` contra las dos vistas viejas, código por código, **antes** de tocar ningún lector | |
| d | migrar los lectores de a uno | |
| e | cuando una tabla no tenga **ningún** lector: backup y borrar | |

Los lectores a migrar (§3.cv):
- **`vista_uni_x_caja`** → `vista_stock_procesada` (pantalla Stock), `vista_generador_oc` (Generar OCs), `vista_importados_partes`
- **`vista_uxb_articulo`** → el Excel de ISIS de Facturación (`_facXlsArmar`)
- **`cob_uxb_lk`** → `vista_facturacion_neto_items`, `vista_plata_perdida`, `cobranzas_precios_super`, `vista_facturable_anticipado`, `gv_articulo_empresa`
- lecturas directas del front a `Articulos_Cajas` y `OC_Maximos`

**Primera candidata a borrarse: `Uni_x_Articulo_x_Caja`** (447 filas). Ya está sin uso: 0 vistas,
0 funciones, 0 referencias en el front. Lo único que la nombra es
...
## §3.cy — Las 7 tablas de UxB con exactamente el mismo dato (v16.18) — 2026-09-12

Thomas: ***"primero quiero que todas las tablas de supa tengan exactamente la misma info, sin
errores"***. Se alinearon las 7 contra `GV_UxB`. Backup fila por fila, con el valor de antes y el de
después, en `public."GV_UxB_Sync_bkp_20260912"`.

| Tabla | Antes (sobre los códigos del listado) | Después |
|---|---|---|
| `proyeccion_madre` | **167 en NULL**, 0 en conflicto | 0 |
| `Articulos_Cajas` | 21 en conflicto | 0 |
| `OC_Maximos` | 6 NULL + 3 en conflicto | 0 |
| maestro (`Articulos Virgilio X Tallerista`) | 6 en conflicto | 0 |
| `Importados` | 2 en conflicto | 0 |
| `cob_uxb_lk` · `precios_venta` | 0, ya estaban bien | 0 |

**205 filas tocadas. Las 7 tablas quedaron en 0 diferencias.**

### ⚠ El cruce se hace por (empresa, código), no por código

`Articulos_Cajas` e `Importados` tienen marca, así que se cruzan **por empresa**. Cruzar sólo por
código habría roto el caso de los dos productos:
...
Un update por código le habría puesto **36 a la pinza de fideos**. Con el cruce por empresa esa fila
queda intacta. Fue el único caso: las 22 candidatas bajaron a 21.

Las tablas sin marca se cruzan por código, y es seguro porque **ningún código tiene UxB distinto
entre LK y CH** en el listado (§3.cx).

### Los cambios que vale la pena mirar

La mayoría ya estaban en la §3.cx. Los nuevos, todos de Chef y todos hacia 12, salen del listado y
conviene confirmarlos:

| | Era | Es |
|---|---|---|
| 909 Noquera Madera **suelta** | 60 | 12 |
| 920 / 922 / 901 / 911 Cuchara Madera **suelta** | 36 | 12 |
| 910 Bate Bife | 36 | 12 |
...
Son los artículos **"SUELTO"** y **"DISPLAY"**: si alguno se compra a granel en otro pack, hay que
corregir el listado, no la tabla.
...
Lista cualquier tabla cuyo UxB no coincida con `GV_UxB`. Si aparece algo: o alguien editó una tabla
vieja a mano, o el **cron 66 `sync-precios-venta`** (cada 15 min, trae de LK) metió otro valor en
`precios_venta` / `cob_uxb_lk`. Esas dos son **las únicas que un cron puede volver a desalinear**;
hoy coinciden.
...
⚠⚠ **Hallazgo al costado, CORREGIDO: `LIBRE` no es basura y NO hay que limpiarlo.** Esta sección
decía que las 54 filas `cod = 'LIBRE'` contradecían la regla 2 del handoff y convenía borrarlas.
**Está mal, y borrarlas rompe cosas.** `LIBRE` es el marcador de **"esta posición está vacía y
disponible"**, y está vivo en cinco lugares: `vista_generador_oc` lo excluye explícitamente dos veces
(`<> 'LIBRE'`); el badge **"posiciones LIBRES"** del mapa lo cuenta y lo muestra; el autocompletado
de la ubicación del excedente **lo prioriza a propósito**; el modal **Mover** valida con él ("el
destino tiene que estar LIBRE o con el mismo código"); y `pmapVieja` lo saltea. La "contradicción"
con la regla 2 tampoco existe: esa regla es para artículos reales, y una posición vacía con
`cajas_max` NULL es exactamente lo correcto. Son 54 filas en 54 sectores, **todos existentes en
`GV_Lugar`** (39 de la góndola P entera con empresa CH, 8 LK, 6 LOKE).

**Lo que sí está mal son 6**, marcadas `LIBRE` pero con algo adentro: **A60** (`989E`, `992E`),
**A65** (`396`, `556`), **C01** (`547`), **C15** (`510T`, `581T`), **Ñ55** (`838E`), y **A83**
(`LIBRE` con `cajas_max = 72` y sin artículo — inconsistente pero inocuo). Las 5 primeras importan:
el badge cuenta libres de más y el autocompletado puede mandar a un operario a dejar un excedente en
una celda ocupada, que es justo lo que el handoff advierte (*"que una celda ACEPTE otro código no
significa que ESTÉ libre"*).

**Causa raíz, con caso testigo:** A60 es la celda que la sesión de Thomas ocupó el 14/09 con `989E` y
`992E`. Le pusieron el artículo y **la marca `LIBRE` quedó** — o sea que `gv_lugar_item_guardar` no
saca la fila `LIBRE` del sector al cargar un artículo. Ése es el bug de fondo; las 6 filas son el
síntoma acumulado. Registrado como problema aparte, **sin tocar ningún dato**.
...
### §3.gc — v18.00: el rename de las tablas PPP dejó a la app VIEJA imprimiendo remitos sin cliente
...
### §3.gd — v18.00: al terminar el armado no se preguntaba más la ubicación (0 eventos AUB desde el 04/09)
...
**El arreglo (v18.00, `index.html`):** `compTerminar()` pregunta la ubicación por sí mismo.
