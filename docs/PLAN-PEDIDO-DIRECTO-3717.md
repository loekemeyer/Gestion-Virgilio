...
**Estado:** **nada ejecutado, salvo el Paso 0, que ya está en producción** (v12.51, commit `24c8a9a` en `main` de `Produccion-Virgilio`; ver §11). No existen `Pedidos_Web`, `np_map`, `ppp_config`, `Facturacion_Export_*` ni las 3 vistas del generador en sombra (verificado hoy con `to_regclass` / `information_schema`: 0 de 4 tablas, 0 de 3 vistas).
...
14. **Correcciones de esta fusión (2026-09-03), las tres verificadas hoy:**
    ...
    - **La partición es por orden ascendente de código de artículo**, no por el orden del carrito. Sin eso, coincide la *cantidad* de tramos pero no el *contenido*, y el renombre 1:1 asigna las líneas a la NP equivocada (§3.3, §4.3).
...
**En cinco líneas.** El pedido web nace en `orders` (LK) y hoy espera hasta 24 h al mail de las 12:30; la operadora lo importa en ISIS, ISIS lo numera, baja dos reportes al Excel PPP, el Apps Script lo empuja a Supabase Virgilio y recién ahí el depósito lo ve. La idea: LK empuja el pedido a Virgilio en minutos, Virgilio lo parte **con el mismo criterio que ISIS** (tope por empresa, en orden de código de artículo), lo programa, lo pickea y lo arma **sin que ISIS exista todavía**, y cuando está armado la operadora baja desde Facturación un Excel **idéntico al de las 12:30 pero con lo armado** y lo importa en ISIS sólo para facturar. ISIS deja de ser la entrada del circuito y pasa a ser la salida. El único dato que ISIS sigue generando y Virgilio necesita es el **número de NP**, que ahora llega *después* de armar y *antes* de facturar (§3).

### 1.1 Hoy vs propuesto, paso a paso

| # | Paso | Hoy (quién) | Propuesto (quién) | Queda / desaparece |
|---|---|---|---|---|
| 1 | Pedido web | Cliente / admin "Pedir para" / cotizador → `submit_order_fast` + `sheets_payload` (`script.js:7047`, payload en `:7252`) | Igual | Queda |
...
| 6 | Partición del pedido en NP (tope **18 LK / 15 Chef**, por código ascendente) | Edge Function LK (`processOrders`) + lo que hace ISIS al importar | Función SQL en Virgilio, **replicando el resultado real de ISIS** (§4.3) | Queda (cambia de lado) |
...
**Entrada del pedido.** `orders.sheets_payload` (jsonb) lo escribe el **navegador** después del RPC, sin `await` (`script.js:7252`) **(v)**. En 90 días: 568 pedidos, 0 sin payload. `orders` no tiene `updated_at`; `status` es `'pendiente'` en 1.236 de 1.236; **no existe cancelación** de un pedido web. `edit_order_fast` bloquea con `FOR UPDATE` + `RAISE` si `enviado_a_compras_at` no es null. Policies vivas en `orders` **(v)**: `orders_delete_own` (DELETE, `auth.uid() = auth_user_id`, **sin mirar sellos**), `orders_update_own_sheets` (UPDATE, ídem), `orders_insert_own`/`_v2`, `orders_admin_*`; triggers: sólo `orders_notify_whatsapp` y `trg_fill_order_customer_code`. `isOrderEditable` (`script.js:1815-1825`) **(v)** hoy da por editable el pedido mientras `enviado_a_compras_at` sea null **y** no haya pasado el corte de las 12:30 que se lo lleva. Las 7 ediciones (`mode='edit'`) de 90 días ocurrieron entre **1 min y 19 h** después del pedido; sólo 2 caen dentro de 5 min. Existe además `rollbackOrder()` (`script.js:6427`), que borra `order_items` y `orders` y hoy **no tiene ningún llamador**: un camino latente que la policy con sello también cierra.
...
**La regla de partición: qué hace la Edge Function y qué hace ISIS.** `processOrders` (`procesar-pedidos-db.ts:6-17`) agrupa por `N° Pedido | Sucursal | Cliente`, y con ≥18 líneas corta en bloques de ≤18 (primero los grandes, después los chicos enteros) numerando `N_Pedido` 1..N del archivo — que **no** es la NP. Pero **lo que termina en ISIS no es exactamente eso**, y es lo que manda para el plan (§3.3):

- **El tope es por empresa: 18 en Loekemeyer, 15 en Chef (v, 2026-09-03).** Sobre `PPP_Base_Pedidos`: LK 700 NP, **253 con exactamente 18 líneas**, 2 con 19 y 25 con 15; Chef 101 NP, **25 con exactamente 15**, 1 con 18 y ninguna por encima de 18. La distribución de Chef (15→25, 14→8, 13→9, 12→6, 11→3, 10→7) tiene meseta inequívoca en 15. Las 2 excepciones de Chef no son del canal web: `44603` es tipo COT (cotizador, 17 líneas) y `44575` ya no está en Programación Diaria. Las 2 NP de LK con 19 líneas tienen **18 artículos distintos** (98293, 98501): es un artículo repetido en el pedido, no un tope de 19.
- **El corte es por orden ASCENDENTE de código de artículo (v, 2026-09-03).** Cada NP contiene un bloque contiguo en ese orden. Cliente 1936, 02/09, 28 ítems → NP `98680` = `031,056E,224,246,315,325,404E,441,501,502,504,505,507,510,513,531,537,544` (18) y NP `98681` = `546,550,562,565,566E,580,587,591,654,812E` (10): corte exacto por código, sin solapamiento. Cliente 989, 02/09, 21 ítems → `98682` = los 18 primeros por código, `98683` = `589E,599E,659`. El `items_string` de `lk_pedidos_match` ya viene ordenado por código por construcción de la vista, así que **no sirve** para deducir el orden del carrito; lo que manda es cómo quedó en ISIS, que es lo de arriba.
- Se cuentan **ítems**, no artículos distintos (fidelidad; consolidar es la pregunta D7). En 60 días: 413 pedidos, 180 con ≥18 líneas, 643 tramos **(v)**.
...
### 3.3 Pre-partición determinística: tope por empresa y orden por código (requisito de A)
...
**(2) La partición es por orden ASCENDENTE de código de artículo, no por el orden del carrito.** Es la corrección más silenciosa del plan: **con el orden equivocado coincide la CANTIDAD de tramos pero no el CONTENIDO**. Un pedido de 28 ítems da 2 tramos con las dos reglas, así que ningún conteo lo detecta; lo que cambia es qué artículo cae en cada NP, y el renombre 1:1 termina colgando líneas armadas, TAL, etiquetas y `Movimientos_Stock` de la NP equivocada — con las cajas de un tramo facturadas contra la otra NP. Verificado sobre casos reales del 02/09 (§2): cliente **1936** (28 ítems → `98680` con `031…544` y `98681` con `546…812E`) y cliente **989** (21 ítems → `98682` con los 18 primeros por código y `98683` con `589E,599E,659`). En los dos, el corte es exacto por código y los bloques son contiguos.

Consecuencia práctica: `aplicar_pedidos_web()` **ordena por código antes de partir** (§4.3.3), y el test `tests/np-particion.sql` compara **contenido tramo a tramo** contra las NP reales de ISIS, no sólo la cantidad.
...
create table "Pedidos_Web" (
  ...
  items          jsonb not null,                    -- [{cod_art, cod_original, cajas, uxb}] tal como vino el payload;
                                                    -- el ORDEN DE PARTICIÓN no es éste: se ordena por código al partir (§4.3.3)
...
3. **Parte replicando lo que ISIS hace al importar (§3.3), no lo que dice `processOrders`:**
   ...
   - **ordena los ítems por `cod_art` ascendente** y corta en bloques contiguos de `tope` líneas. `parte` = 01..N. Con `count(items) <= tope` es un solo tramo.
   - Cuenta **ítems**, no artículos distintos (fidelidad; consolidar es la pregunta D7). El orden dentro del tramo que viaja al Excel es el mismo orden por código.
...
|---|---|---|
| `armada` | Trigger AFTER INSERT en `Entregas_Virgilio` (`SECURITY DEFINER`) cuando la NP web ya tiene filas con `cajas_entregadas > 0`. Un tramo armado **con todo faltante** (0 entregadas) no llega a `armada`: queda listado aparte como "armado sin nada que facturar" y se resuelve con `NP_Canceladas` o esperando el completado (D20) | Aparece en el checklist "Armadas — pendientes de ISIS" de Facturación (§11.2). **No** escribe `Facturacion_NP` |
...
- **Borrado o reescritura por el cliente después del push:** en la **fase 2**, `alter policy orders_delete_own … using (auth.uid() = auth_user_id and enviado_a_virgilio_at is null)` y lo mismo en `orders_update_own_sheets` (`using` y `with check`). Hoy las dos policies **no miran ningún sello** (v); además existe `rollbackOrder()` (`script.js:6427`) sin llamador, que la policy también cierra.
...
| 2 | `N_Pedido` | Correlativo 1..N **por TRAMO**, no por NP: cada NP se corta de a **18 líneas (LK) o 15 (Chef)**, en orden de código, y cada bloque lleva su `N_Pedido` | — | Ídem; 1 tramo = 1 `N_Pedido` = 1 NP de ISIS; los complementos llevan el suyo | El tope y el orden son lo que hace que el mapeo `NP_i = primera_NP + (N_Pedido_i − 1)` sea 1:1 (§3.3) |
...
|---|---|---|---|---|
| **0 — HECHA (v12.51, `24c8a9a`)** | Botón **"⬇ Excel ISIS (prueba)"** en el cierre de Facturación, sólo supervisor, **100 % en el navegador**: cero funciones, tablas o columnas nuevas en Supabase. Lee `Facturacion_NP`, `Entregas_Virgilio`, `PPP_Base_Pedidos`, `vista_uxb_articulo`, `clientes_vendedor` y `lk_pedidos_match` con la anon key; dedup por `(NP, artículo)` quedándose con la última fila y topeando contra lo pedido; **split 18 (LK) / 15 (Chef)** y **orden por código ascendente**; archivo `PRUEBA_NO_IMPORTAR_DD-MM-YY_HHMM.xls` con la advertencia en la hoja "Resumen". Test `tests/fac-excel-isis.cjs` en `tests/run.sh` | **Cumplido en código y test** (suite en verde). **Pendiente lo que no es código:** (a) la **prueba real de importación con la persona de ISIS** (H1-H7); (b) `leyenda2` y `condPago` salen **vacíos** porque viven sólo en la base de LK → es exactamente la **pregunta D11** | S | — |
...
| **3** | Virgilio: `aplicar_pedidos_web()` (validaciones, **partición con tope por empresa y orden por código**, `reenviado`, `cancelado`) + cron `*/2` + watchdog; `pppFindNpPdf`; prefijo visual "NPV" | Un pedido de prueba genera N tramos en `PPP_*` con `origen='web'`; **`tests/np-particion.sql`**: sobre los pedidos de 60 días la función da los mismos tramos que las NP reales de ISIS **en cantidad y en contenido**, con casos LK de 19+ líneas y **un caso Chef de 16-17 líneas** (el que rompería con tope 18); m³ dentro del 5 % de ISIS; zona derivada; la PPP sobrevive a un push del Sheet; una fila inválida queda en `error` con aviso y **no** entra a la PPP | M | Sí (con `Pedidos_Web` sembrada a mano) |
...
| Vista | Qué hace |
|---|---|
| `v_shadow_web_items` | Parsea `lk_pedidos_match.items_string` a `(art, cajas)` y numera cada línea con `row_number() over (partition by order_id order by art)` — o sea **en orden de código**, con `cap_lineas = case when empresa='chef' then 15 else 18 end` |
...
**Límite honesto de esa validación — hay que decirlo.** La comparación es **por cliente-día** y valida **cuántas NP y cuántas cajas**, no **qué artículo cae en cada NP**. El número de tramos y el total de cajas son **independientes del orden** en que se parte, así que ese 89/89 **NO prueba** la corrección de §3.3 punto 2 (partición por código ascendente): un generador que partiera en el orden del carrito daría exactamente el mismo 89/89 y estaría mal. **Extender la comparación al contenido tramo a tramo** — códigos de cada NP generada contra los de la NP real de ISIS — es trabajo de esta fase, y es lo que después reusa `tests/np-particion.sql` (fase 3). Además, el generador de hoy **no aplica el corte por sucursal** (≈9 % de los cliente-día tienen más de un pedido, y falta decidir si ISIS los junta o los separa) y **no cubre el canal no-web** (esos cliente-día salen como `solo_isis`, que no es un error sino lo esperado). Los otros bordes a cerrar en sombra: cargar los 8 artículos sin volumen, limpiar las direcciones mezcladas cliente/expreso y definir con precisión qué cuenta como "línea".
...
| **11** | **La partición es por código ascendente y no por orden del carrito** (v). Con el orden equivocado coincide la cantidad de tramos pero no el contenido | Líneas armadas, TAL, etiquetas y `Movimientos_Stock` colgando de la NP equivocada, y cajas facturadas contra otra NP — **sin que ningún conteo lo detecte** | `aplicar_pedidos_web()` ordena por código antes de partir; el test compara **contenido tramo a tramo**, no cantidad; la fase 0-bis extiende su comparación al contenido antes del piloto (§7.1) |
...
3. **Partición.** ¿Mantener el corte en Virgilio al ingresar — **tope por empresa (18 LK / 15 Chef) y orden por código ascendente**, 1 tramo = 1 unidad de armado = 1 NP (**recomendada**) —, o armar el pedido entero y partir sólo al exportar (rompe el renombre 1:1)?
...
21. **¿El piloto arranca por Loekemeyer o por Chef?** (pregunta nueva). Chef es **más chico** — 101 NP contra 700 en Base, 13-15 pedidos web contra 141-150 en Programación —, así que un error tiene menos alcance, y es el argumento de la propuesta. **Recomendación: arrancar por Loekemeyer**, por tres razones concretas: (i) el tope de Chef es **15** y esa regla está verificada sobre **25 NP**, contra las 253 que sostienen el 18 de LK — es evidencia diez veces más fina para el parámetro del que depende la correspondencia 1:1; (ii) Chef tiene **la sucursal sin resolver en el 92 % de sus filas** (50 de 57 sin `sucursal_entrega`, por el camelCase de §4.11) y el vendedor en el 38 %, o sea que su Excel todavía no se puede armar completo; (iii) Chef depende de **precondiciones abiertas en un tercer proyecto Supabase** (`nkhzocgdpwtgrmwleihr`) que no es consultable desde acá: no se sabe si tiene su propio cron de "procesar pedidos", ni está confirmado el grant para sellar sus `orders`. En LK está todo verificado y los tres puntos son cero riesgo. Si el dueño prefiere igual empezar por Chef, el orden mínimo es: cerrar (i), (ii) y (iii) primero — o sea hacer la fase 8 antes que la 6, que es más trabajo, no menos.
...
| **Orden de la partición (v, 2026-09-03)** | Ascendente por código de artículo, bloques contiguos. Cliente 1936 → `98680` (`031…544`, 18) + `98681` (`546…812E`, 10); cliente 989 → `98682` (18 primeros) + `98683` (`589E,599E,659`) |
...
## 11. Paso 0 — botón Excel en Facturación: **HECHO y en producción (v12.51)**

**Estado: desplegado.** Commit `24c8a9a` en `main` de `Produccion-Virgilio`, `APP_VERSION = "v12.51"` / `SW_VERSION = "v12.51-vir"`. Es lo único de la idea 3717 que existe hoy; **todo lo demás de este plan sigue sin ejecutar**.
...
**Los tres candados (el archivo no se importa en producción).** Las NP que lleva ya están numeradas y facturadas en ISIS; si se importara, ISIS crearía NP nuevas para pedidos que ya existen. Por eso: (1) el archivo se llama **`PRUEBA_NO_IMPORTAR_DD-MM-YY_HHMM.xls`**; (2) la hoja "Resumen" abre con **"PRUEBA DE FORMATO — NO IMPORTAR"** y lista las NP reales incluidas; (3) el módulo lo abre **sólo un supervisor** (`requireSupervisor()`, `index.html:24694`) — el botón vive visible en el bloque `.fac-cierre` (`:3485`), sin el gate por `?isisTest=1` que proponía el borrador, porque el gate de supervisor ya alcanza y evita un parámetro de URL que hay que recordar.

### 11.1 Cómo está implementado (100 % navegador, cero backend nuevo)

**No crea ninguna función, tabla ni columna en Supabase.** Lee con la anon key las mismas tablas que ya lee la app: `Facturacion_NP` (NP facturadas en el rango de fechas elegido), `Entregas_Virgilio` (líneas armadas), `PPP_Base_Pedidos` (fecha del pedido), `vista_uxb_articulo` (`uxb`), `clientes_vendedor` (`vend`) y `lk_pedidos_match` (sucursal). Todo en `index.html`, funciones `facXlsIsisOpen` / `facXlsIsisCargar` / `_facXlsRender` / `_facXlsXml` / `facXlsIsisGenerar` (`:32885-33290`).

Reglas implementadas, las tres verificadas contra datos reales el 2026-09-03:

- **Tope por empresa:** `_facXlsEmpresa(np)` mira el **primer dígito** (9 = LK, 4 = Chef) y `_facXlsTope(np)` devuelve **18 o 15**. La columna "NP en ISIS" del checklist muestra en cuántos tramos se va a partir cada NP, y las de Chef llevan el aviso "Chef (corta de a 15)".
- **Orden por código ascendente:** las líneas de cada NP se ordenan por `articulo` (ya normalizado con `padCodArt`) **antes** de cortar en bloques de `tope`, y cada bloque recibe su `N_Pedido` correlativo.
- **Dedup por `(NP, artículo)`:** se toma la **última** fila cargada (mayor `id`) y se topea con `least(entregadas, pedidas)`; se marca ⚠ "re-armada" si había más de una, y la NP cuya suma de entregadas supera lo pedido arranca **destildada**. Sin esto, 28 NP de 748 habrían ido al doble.
- Avisos del checklist: `entregado > pedido` (rojo, destilda), `re-armada`, `sin sucursal`, `sucursal dudosa`, `sin vendedor`, `sin unidades por caja`, `Chef (corta de a 15)`.

**Formato:** port literal de `generateExcel` — XML Spreadsheet 2003, mismos estilos `Default/Header/Data/Desglose`, hoja 1 `"DD-MM-YY 9Hs"` sin encabezado con las 12 columnas en orden, misma regla `isNum` (un `String` que empieza con `0`, como `029`, queda String), vacío → `<Cell/>`; hoja 2 "Resumen" con la advertencia, `Pedidos | NP` y el desglose `Cod Clte | Num Ped | Cant Items`.

**Test:** `tests/fac-excel-isis.cjs` (158 líneas) en `tests/run.sh` — cubre dedup, split 18/15, orden por código, XML Spreadsheet 2003 y el nombre del archivo. Suite completa en verde.

### 11.2 Qué quedó pendiente del Paso 0

1. **La prueba real de importación con la persona de ISIS.** Es lo único que el Paso 0 no puede resolver solo: hay que sentarse con Horacio (o con la operadora, en un entorno de prueba) y responder H1-H7 — en particular si el importador acepta la fecha vieja (H3), los códigos reales tipo `437E` (H7) y qué pasa con un pedido importado y no facturado (H2). **El archivo está listo desde hoy.**
2. **`leyenda2` y `condPago` salen vacíos.** No es un olvido: la leyenda "D x - LC x - PP x" (deuda, límite de crédito, plazo) y el **código** de condición de pago **viven sólo en la base de LK** — `sheets_payload.condicion_pago_code` los tiene en el 100 % de los pedidos, pero `lk_pedidos_match`, que es lo único que Virgilio ve, **no lleva esa columna** (verificado hoy). Traerla es 1 columna en `v_pedidos_match` + `lk_pedidos_match` con la misma sync de 15 min: **es la pregunta D11**, y hasta que se responda la prueba con ISIS mide un 0 % ficticio en esas dos columnas (o confirma, vía H5, que no son obligatorias).
3. **Chef sale sin sucursal.** 50 de 57 filas chef de `lk_pedidos_match` vienen sin `sucursal_entrega` (causa: camelCase en el payload de Chef, §4.11), así que las NP `4…` salen con la sucursal vacía y el aviso correspondiente. No bloquea la prueba de formato de LK.
4. **Sin persistencia del estado "exportada"** (marco (b)): el módulo no escribe nada. La fase 5 agrega `Facturacion_Export_ISIS` + `Facturacion_Export_Lineas` y mueve la consulta a la RPC `facturacion_export_isis()` (§4.6), con el score de sucursal y la agregación por tanda vigente en lugar de las versiones simples de hoy.
5. **El campo "primera NP de ISIS" y `confirmar_np_isis()` no existen** y no corresponden al Paso 0: son fase 5.
...
1. ~~Paso 0 en el repo actual, con su test.~~ **Hecho** (v12.51). Falta la prueba de importación con ISIS y la decisión D11.
2. **Fase 0-bis:** desplegar `sql/ppp_shadow_generator.sql` (3 vistas read-only), extender la comparación al **contenido tramo a tramo**, y dejarlo corriendo semanas hasta el empate sostenido. No toca nada.
...
| OP-3 | alta | NP armadas dos veces: las cajas entregadas sumadas duplican la factura | **Aceptada**, y el diseño va **más lejos** que lo que pedía el crítico: no alcanza con "tomar la última fila". Se agrega por `(np, cod_art)` **sumando dentro de la tanda vigente**, con `least(·, pedidas)`, ⚠ y bloqueo si el total supera lo pedido. Coincide con DA-1 | §4.6, §11.1, riesgo 20 |
...
| OP-9 | media | Carrera de edición: el push lleva el payload viejo y `version > 1` es código muerto | **Aceptada** en el mecanismo (des-sello + `version_virgilio` + triggers), pero **con el hash corregido**: no puede compararse por código, porque 102 de 570 pedidos (18 %) tienen códigos distintos entre payload y `order_items` por la sustitución de variante. La condición real es **cantidad de líneas + suma de cajas** (0 de 570 difieren). Se **descarta** armar los ítems desde `order_items`: el payload lleva `cod_original` y `uxb` tal como viajan hoy | §4.2.2, §4.9 |
...
| DA-8 | media | Chef en el Paso 0 sale sin sucursal en más del 90 % y el plan lo tapaba con el promedio global | **Aceptada** (v: 50 de 57 filas chef sin sucursal; 65 NP Chef de 450). Cobertura por empresa en §5 y §10, y la sucursal en `v_pedidos_match_chef` como precondición de la fase 8. **Se agrega** que el vendedor tiene el mismo sesgo: LK 99 % contra Chef 38 % | §5, §10, §4.11 |
...
| DA-13 | baja | "Ninguna tabla nueva" en el Paso 0 no era cierto | **Aceptada**, y el Paso 0 desplegado la cumple con creces: **0 tablas, 0 columnas y 0 funciones** en Supabase — todo el armado quedó en el navegador | §11.1 |
...
| TE-15 | baja | El plan no suma nada a `tests/run.sh` y el Excel se valida a ojo | **Aceptada y ya cumplida en la parte 0**: `tests/fac-excel-isis.cjs` está en `run.sh` desde v12.51. Faltan `emp-np.cjs` ampliado y `anon-writes.cjs` (fase 1), `np-particion.sql` (fase 3) y `ppp-programar-rpc.cjs` (fase 4) | §7, §11.1 |
...
| **F-1** | **alta** | **El tope de líneas por NP no es 18 para las dos empresas: es 18 en Loekemeyer y 15 en Chef** | **No vino de los críticos**: salió de la rama de implementación en sombra y se **re-verificó hoy** contra `PPP_Base_Pedidos` (LK: 253 de 700 NP con exactamente 18; Chef: 25 de 101 con exactamente 15, ninguna por encima de 18). El plan asumía 18 en §3.3, §4.3, §5, §7, §8, §10 y §11. **Aceptada:** el tope pasa a ser **parámetro por empresa** (`ppp_config.tope_lineas_np`), nunca un literal. Consecuencia dicha en voz alta: en Chef, partir de a 18 **rompe la correspondencia 1:1 tramo↔NP** de la que depende todo el renombre — un pedido Chef de 16 líneas daría 1 tramo en Virgilio y 2 NP en ISIS | §3.3, §4.3.3, §4.11, §5 col 2, §7 f3/f8, riesgo 10, §10, §11.1 |
| **F-2** | **alta** | **La partición es por orden ascendente de código de artículo, no por el orden del carrito** | **No vino de los críticos**: el plan copiaba el "bloques de 18 en el orden del payload" de `processOrders`, pero lo que ISIS produce es otra cosa. Verificado hoy sobre NP reales del 02/09 (clientes 1936 y 989): cada NP es un bloque **contiguo en orden ascendente de código**. **Aceptada:** `aplicar_pedidos_web()` ordena por código antes de partir. Por qué importa: con el orden equivocado **coincide la cantidad de tramos pero no el contenido**, así que ningún conteo lo detecta y el renombre 1:1 cuelga las líneas de la NP equivocada | §3.3, §4.3.3, §5 col 2, §7 f3, riesgo 11, §11.1 |
...
Líneas verificadas contra HEAD (`24c8a9a`, v12.51) el 2026-09-03. **Ojo:** el Paso 0 agregó ~398 líneas en `index.html` alrededor de la línea 32885, así que todas las referencias posteriores a ese punto se corrieron respecto de versiones anteriores de este plan.

| Archivo / objeto | Líneas / nombre | Fase |
|---|---|---|
| `index.html` **Paso 0 (ya desplegado)** | Botón `facBtnXlsIsis` `:3485`; bloque `facXlsIsisOpen` `:32942`, `facXlsIsisCargar`, `_facXlsRender`, `_facXlsXml`, `facXlsIsisGenerar` (`:32885-33290`); helpers `_facXlsPadCod`, `_facXlsEmpresa`, `_facXlsTope` | **0 (hecha)**, 5 (migra a RPC) |
| `index.html` Facturación | `.fac-cierre` (CSS `:3069`, bloque `:3492`); `facRenderTicked` `:34172`; `facAuthWriteHeaders` `:34353`; `_facEsOperadora` `:34372`; `facFCEnsureModal` `:34429`; `window.facTickNP` `:34559`; `generateFacturacionPDF` `:34858` | 0, 5 |
| `index.html` PPP | `PPP_READONLY` (`:28132` el bloque, usos en `:25550`, `:26688`, `:27559`); `pppRenderProg` `:28213`; `pppAutoBaseN`; `_pppComputeSugerencia`; `_pppScheduleTandas` `:26453`; `pppConfirmarProgramar` `:26506`; `PPP_EDITS_KEY` `:26146`; `PPP_CFG_KEY` `:26171`; `pppSubir` `:28425`; `pppFindNpPdf` `:26827`; `pppTandaM3Map` `:25716`; `_pppMetaEntSet` `:25741`; `pppEsSuper` `:25568` | 4, 5, 9 |
| `index.html` stock y drenaje | `stockSalidaFacturadoNP` `:22782` (**no se toca** en esta idea); `stockDrenarCPFacturado` `:22812` | — |
| `index.html` empresa por NP | `empresaDeNp` `:7371`; `pkCodEmpresa` `:7376`; `pkNpEsLoeke` `:8035`; `_pppOrderDirsForNp` (ya usa el primer dígito, **no se toca**) | **1** (en los dos repos) |
| `index.html` carga de camión, CP y ruteo | Pantalla de CCN (guard de NPV); CP y RC (bloqueo sobre las exportadas); `openRuteo` `:28023`; `stkOpenNpFaltan` | 5, 7 |
| `index.html` monitor y cola offline | `fetchMonitorFromSupabase` `:30600`; `enqueueReport` `:6247` y `QUEUE_KEY` `:6139` (por qué el guard no usa `RAISE`) | 5 |
| `index.html` identidad | `SUPERVISOR_EMAILS` (gate en `:24691`); `requireSupervisor` `:24694`; `window.sbAuth` | 0, 4, 5 |
...
| SQL de LK | `submit_order_fast`, `edit_order_fast`, policies `orders_delete_own` y `orders_update_own_sheets`, `sync_pedidos_match_virgilio` con `v_pedidos_match` / `v_pedidos_match_chef` (`sql/pedidos_match_virgilio.sql`; columna `condicion_pago_code`), `enviar_pedidos_main`, `postear_envio_pedidos`, `sync_order_tracking_from_sheet`, cron `procesar-pedidos-web`, server `virgilio_db` | 0 (D11), 2, 8, 9 |
| Front de LK | `script.js:1815` `isOrderEditable`; `:1633` la búsqueda de tracking por `order_id`; `:6427` `rollbackOrder` (sin llamador); `:7047` `_submitSingleOrder` y `:7252` la escritura del payload; `admin.js:2413` `parseTrackingSheet` | 2, 6, 7 |
...
| SQL nuevo (fase 0-bis) | `sql/ppp_shadow_generator.sql` — `v_shadow_web_items`, `v_shadow_np_gen`, `v_shadow_ppp_compare` (**no desplegado**) | **0-bis** |
| Tests | `tests/run.sh`; **`fac-excel-isis.cjs` ya integrado**; nuevos `np-particion.sql` (contenido tramo a tramo, caso Chef de 16-17 líneas), `ppp-programar-rpc.cjs`, `anon-writes.cjs`; `emp-np.cjs` ampliado | 0 (hecha), 1, 3, 4 |
