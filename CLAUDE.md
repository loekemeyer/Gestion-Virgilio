...
- **NP** → **la NP web ES el número de pedido de la página** (v12.92, dueño: *"ya cuando
  ...
  **v13.47 (domingo 06/09, dueño: *"mandá directo a Programación si ya está, no más en A Programar"*):
  ...
  sólo Retira, Súper, sin zona y sin camión previsto. **v13.61: A Programar es una PLANILLA** (una fila por NP, formato ISIS): tildar → "Generar tanda … para el [día]" (código automático, `gv_ppp_web_tanda_nueva` → `_agregar` → `_programar`, se descarta si falla): **ninguna tanda queda sin fecha**. Y **la PPP entra entera en una pantalla** (`pppFitPantalla`, zoom automático, sin scroll interno). Tandas numeradas como Producción (E01A, E02A…). **v13.60: camión = LETRA+NN y se REUSA por día y etiqueta** (Capital / GBA Sur / GBA Oeste / GBA Norte): una tanda nueva entra al camión que ya va ese día, aunque sea de ISIS (E01F, D68G); camión nuevo sólo si no hay. `sql/gv_ppp_web_camion_del_dia.sql`, §3.at. Ese día además se corrió la semana un día hábil (lun 7 feriado, nada armado; súper quedan) — backup `sql/backups/reprogramacion_20260906_pre_v1360.sql`.
