-- v14.75 (2026-09-10) — Que los 6 writers de Movimientos_Stock PASEN la columna `empresa`
...
begin
  ...
  loop
    ...
    -- v14.75: empresa explícita (para que bajar un rack CH caiga en la góndola CH, no en Mixto)
...
BEGIN
  ...
  -- v14.75: empresa del sector contado (góndola). Sin sector o insumo → NULL (trigger decide).
...
begin
  ...
  if coalesce(btrim(r.detalle), '') <> '' then
    ...
    select cod, 'a_guardar', -cajas, 'ajuste',
           ...
           'anrec_' || p_id || '_' || cod,
           nullif(btrim(r.linea), '')                     -- v14.75: empresa de la recepción original
      from (
...
begin
  ...
  if p_accion = 'descontar' then
    -- v14.75: empresa de la góndola que drenó esa tanda+cod (una sola empresa → inequívoco)
