-- ═══════════════════════════════════════════════════════════════════════════════════════
-- v13.60 (2026-09-06, domingo a la noche) — UN CAMIÓN POR DÍA Y ZONA, NO UNO POR CORRIDA
-- (migraciones aplicadas como *_v1359; el número v13.59 lo tomó el otro chat que apagó los crons 71/73)
-- ═══════════════════════════════════════════════════════════════════════════════════════
-- Dueño: "el viernes 11 hay cinco camiones, no puede ser. El 14 veo cuatro camiones. No tiene
-- sentido que se parta así los días de programación."
--
-- Causa: `ppp_web_armar_tandas` numeraba cada corrida con un camión NUEVO (E01, E02, E03…), así
-- que el intradía (cada 15 min) abría un camión por cada pedido que caía: el vie 11 quedó con
-- D68 (ISIS) + E01 + E02 (0,55 m³) + E04 (0,11 m³) + E07 (Chango Mas). Camión = LETRA+NN
-- (E01A + E01B = camión 01, igual que el tablero y que Producción).
--
-- Regla nueva (misma que ISIS: D60 = zonas 4+5, D67 = 1+2+3, D69 = 5+6):
--   · Antes de numerar, se miran los camiones que YA van ese día (web + ISIS, sin súper ni retira)
--     y a qué "camión" pertenece cada parada (`gv_ppp_web_camion`: Capital · GBA Sur · GBA Oeste ·
--     GBA Norte, por sector/zona). Una tanda nueva para esa etiqueta entra al camión existente
--     con la próxima letra (E01A–E01E → E01F; D68A–F → D68G, aunque el camión sea de ISIS).
--   · Sólo si no hay camión a esa etiqueta ese día, abre uno nuevo con el número siguiente.
--   · Súper: siempre camión propio (sin cambios). Retira: sin camión.
--   · Todo código nuevo pasa por `gv_ppp_web_codigo_tomado` (que ahora mira también la tabla de
--     override, donde vive E07A): nunca repite uno que exista.
--
-- Datos del mismo día (backup: sql/backups/reprogramacion_20260906_pre_v1360.sql):
--   · Reprogramación "un día hábil adelante, salvo súper" (lun 7 feriado, nada armado):
--     D60A–F 8→9, D66A–F 9→10, D67A–K 10→11, D68A–F 11→14, D69A–C 14→15 (81 NP, vía
--     GV_PPP_Prog_Override.fecha_entrega, nota 'v13.60 …'); web E01A–E/E02A/E04A 11→14,
--     E03A/E05A/E06A/E08A 14→15. Súper quedan: D59A Coto y D61A Carrefour (8, armadas), D62A
--     Patagonia (9), E07A Chango Mas (11). D71A Matiz (16) no se tocó.
--   · Consolidación: E02A→E01F, E04A→D68G, E05A→D69D, E06A→D69E, E08A→E03B.
--
-- Rollback: ejecutar el backup (fechas y códigos) y volver a aplicar
-- sql/gv_ppp_web_armar_pendientes.sql §3 y §4 (letra_y_camion y armar_tandas v5).
-- ═══════════════════════════════════════════════════════════════════════════════════════

-- ── 1. Un código ocupado en la tabla de override también cuenta ──────────────────────
...
returns boolean language sql stable set search_path = public, pg_temp as $$
  select exists (select 1 from public."PPP_Programacion_Diaria" where tanda = p_codigo)
      ...
      or exists (select 1 from public."GV_PPP_Prog_Override"    where tanda = p_codigo);   -- v13.60
$$;

-- ── 2. La letra/último camión también mira el override (E07A vive sólo ahí) ─────────
...
begin
  select public.ppp_web_letra_idx(m[1]) as idx, (m[2])::int as nn
    ...
    from (
      ...
      union all
      select regexp_match(tanda, '^([A-Z]+)([0-9]+)[A-Z]+$') from public."GV_PPP_Prog_Override" where tanda ~ '^[A-Z]+[0-9]+[A-Z]+$'   -- v13.60
    ) t
...
-- ── 3. ppp_web_armar_tandas v6: reusa el camión del día ─────────────────────────────
-- Cuerpo = el vivo al 2026-09-06 (v5, v13.47) con los cambios marcados "v13.60".
...
-- ── 3b. Un súper con zona numérica no es camión a esa zona ──────────────────────────
-- E07A Chango Mas quedó en ISIS como "Zona 5 - GBA Oeste" (tipo KRIKOS): gv_ppp_web_dia_camion lo
-- tomaba como camión a la zona 5 el vie 11 y el intradía mandaba ahí un pedido de Morón (E08A).
-- Migración gv_ppp_web_dia_camion_sin_super_v1359.
  ...
  select min(dia) from (
    ...
    select left(btrim(i.fecha_entrega::text), 10)::date
     ...
     where zn.n is not null
       ...
       and coalesce(nullif(btrim(i.tanda), ''), '') <> ''
       and coalesce(i.tipo, '') <> 'KRIKOS'   -- v13.60
       and (regexp_match(btrim(coalesce(i.zona, '')), '^Zona\s*([0-9]+)'))[1] = zn.n
  ) d;
$$;

-- ── 4. Prueba sin escribir ──────────────────────────────────────────────────────────
-- select * from public.gv_ppp_web_armar_simular('lk', '2026-09-14', '[{"order_id":9001,"np_idx":1,
--   "cod":"T1","razon_social":"Prueba","zona":"Zona 1 - CABA Sur","barrio":"Barracas","m3":"0.3"}]');
-- Probado el 2026-09-06 con gv_ppp_web_armar_pendientes_simular('lk','2026-09-14', filas, '[]'):
--   zona 1 Barracas → E03C (mar 15, el 14 estaba lleno) · zona 6 Tigre → D69F (camión D69 de ISIS) ·
--   zona 5 Morón → D68H (camión D68 de ISIS, lun 14). Sin rastro en PPP_Web_Programacion.
