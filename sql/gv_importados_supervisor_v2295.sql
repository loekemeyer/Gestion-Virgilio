-- v22.95 (2026-09-26) — IMPORTACIÓN: escribir sólo con login de SUPERVISOR (cierra la v22.81)
...
--      marca 'v22.95-sup'; falla con raise si una no matchea). Pasa: supervisor (es_supervisor_virgilio) o
--      servicio/postgres (gv_es_supervisor_o_servicio: crons, MCP, service_role).
--   4) Un centinela por RPC en GV_Reglas_Centinela.
--   El front (v22.95) ya firma estas RPC con la sesión Google (_pedImpRpc + _PED_IMP_RPC_ESCRITURA).
--
...
declare
  ...
  guard constant text :=
    E'\n  -- v22.95-sup: sólo supervisor (Thomas 26/09, cierra la v22.81)\n'
...
begin
  foreach f in array array['gv_imp_carga_pedido_set','gv_imp_cc_deuda_add','gv_imp_cc_deuda_borrar','gv_imp_cc_deuda_set',
    ...
    if position('v22.95-sup' in d) > 0 then continue; end if;          -- ya aplicado
...
select f, 'funcion', 'es_supervisor_virgilio\(\) or public\.gv_es_supervisor_o_servicio\(\)',
 ...
 where not exists (select 1 from public."GV_Reglas_Centinela" c where c.objeto = f and c.version = 'v22.95');
...
-- Candado: por cada función, pg_get_functiondef → sacar el bloque que empieza en '-- v22.95-sup' hasta su 'end if;' → execute.
-- update public."GV_Reglas_Centinela" set activo = false where version = 'v22.95' and patron like 'es_supervisor_virgilio%';
