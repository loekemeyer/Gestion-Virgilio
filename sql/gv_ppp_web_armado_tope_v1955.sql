...
begin
 ...
 || E'    with z as (select x, row_number() over () ord from jsonb_array_elements(p_filas) x),\n'
 || E'         g as (select btrim(coalesce(z.x->>''cod'','''')) cod, min(z.ord) ord0, count(*) n from z group by 1),\n'
 || E'         k as (select count(*)::int kk from g),\n'
 || E'         r as (select g.*, (row_number() over (order by g.ord0))::int ix from g),\n'
 || E'         o as (select r.*, ((r.ix - 1\n'
 || E'                 - (floor(extract(epoch from now()) / 300)::bigint % (select kk from k))::int\n'
 || E'                 + (select kk from k)) % (select kk from k)) rot from r),\n'
 || E'         a as (select o.cod, o.rot, sum(o.n) over (order by o.rot) hasta from o),\n'
 || E'         e as (select a.cod from a where a.hasta <= v_tope\n'
 || E'               union select a.cod from a where a.rot = 0)\n'
 || E'    select coalesce(jsonb_agg(z.x order by z.ord), ''[]''::jsonb) into p_filas\n'
 || E'      from z where btrim(coalesce(z.x->>''cod'','''')) in (select cod from e);\n'
