-- ════════════════════════════════════════════════════════════════════
-- GV_Lugar / GV_Lugar_Item / gv_ocupacion_lugar  (v1, 2026-09-11)
...
create table if not exists public."GV_Lugar_Item" (
  ...
  created_at  timestamptz not null default now(),
  updated_at  timestamptz not null default now(),
  primary key (sector, cod)
);
comment on table public."GV_Lugar_Item" is
  'Asignación lugar <-> artículo/insumo (muchos a muchos). `cajas_max` vive acá y no en GV_Lugar porque 53 códigos tienen capacidad distinta según el sector. `clase` unifica artículos e insumos en una sola tabla. v1 2026-09-11, todavía sin lectores.';

create index if not exists gv_lugar_item_cod_idx   on public."GV_Lugar_Item" (cod);
