...
(async () => {
  ...
  const r = await p.evaluate(async () => {
    ...
    // v18.00 — el asistente SÍ pregunta la ubicación (1 vez): lo que no puede pasar es que el
  ...
  const checks = [
    ...
    ["6124: el asistente pide la ubicación UNA vez (v18.00)",   r.askUbicPrimero === 1 && r.stockSepPrimero === 1],
