...
echo "== rcp-empresa-dual (v22.37: la entrega del tallerista graba la empresa) =="
node tests/rcp-empresa-dual.cjs
echo "== apr-lotes-carrera (v22.36: una carga por lote en vuelo no pisa la invalidación) =="
node tests/apr-lotes-carrera.cjs
echo "== art-nombres-carga (v22.32: el padrón de nombres carga de verdad) =="
node tests/art-nombres-carga.cjs
...
echo "== ppp-barrios-sin-sector (v19.31: un barrio sin sector no vuelve a la regla vieja de 7 zonas) =="
...
echo "== comp-ent-evento (v17.85: el armado va TAMBIEN por la cola de eventos, para que el servidor lo pueda rehacer sin el celular) =="
PLAYWRIGHT_BROWSERS_PATH="${PLAYWRIGHT_BROWSERS_PATH:-/opt/pw-browsers}" node tests/comp-ent-evento.cjs
...
echo "== apr-badge-horario (v17.74: badge de horario en A Programar para super, los 3 aparte y los que retiran) =="
PLAYWRIGHT_BROWSERS_PATH="${PLAYWRIGHT_BROWSERS_PATH:-/opt/pw-browsers}" node tests/apr-badge-horario.cjs
...
echo "== rest-tope-wrapper (regresión: el envoltorio de fetch que tapa el tope de 1.000 filas) =="
node tests/rest-tope-wrapper.cjs

echo "== pga-salio-badge (regresión: la pastilla y el color netean lo que salió) =="
PLAYWRIGHT_BROWSERS_PATH="${PLAYWRIGHT_BROWSERS_PATH:-/opt/pw-browsers}" node tests/pga-salio-badge.cjs
...
PLAYWRIGHT_BROWSERS_PATH="${PLAYWRIGHT_BROWSERS_PATH:-/opt/pw-browsers}" node tests/apr-contenido-np.cjs
...
echo "== ppp-jornada-camion (v16.65: tope de 8 h por camión Y por día repartido entre los fleteros) =="
...
echo "== feriados-canonica (v21.47: los dos fronts bajan public.GV_Feriados y conservan su fallback) =="
node tests/feriados-canonica.cjs

echo "== mon-vs-vista (v21.47: 7 numeros, hs_prod incluido: el monitor grande y la vista de la TV dan el MISMO numero) =="
...
PLAYWRIGHT_BROWSERS_PATH="${PLAYWRIGHT_BROWSERS_PATH:-/opt/pw-browsers}" node tests/regla-L-super.cjs
