...
(async () => {
  ...
  const r = await p.evaluate(async () => {
    ...
    out.cols = ["Día", "m³", "Tandas", "NPs", "Facturado", "Armado", "En proceso", "Pendientes"]
      .every((c) => new RegExp(">" + c.replace("³", "³") + "<").test(html));
  ...
  t(r.cols, "(1) columnas Día · m³ · Tandas · NPs · Facturado · Armado · En proceso · Pendientes");
